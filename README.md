...
| слот | файл | публичный RMSLE |
| :--- | :--- | ---: |
| 1 | `artifacts/submission_v37_hull2d.csv` | 1.6472485 |
| 2 | `artifacts/submission_final_hedge33.csv` | 1.6475667 |

**Проверка воспроизводимости — одна команда, данные не нужны:**

```bash
python scripts/verify_finals.py
```

Скрипт пересобирает каждый шаг цепочки из сохранённых направлений
(`artifacts/neural/*_a408.npz`) и сверяет с записанными CSV. Все шаги
сходятся с точностью $5.6\cdot10^{-17}$.

**Источники данных.** Используется только `data_start/train.parquet` и
`data_start/sample_submit.csv` из комплекта соревнования. Внешних данных,
скачиваний по сети и чужих предобученных весов в коде нет — все пути
объявлены в [src/ecup/config.py](src/ecup/config.py), сетевые вызовы
отсутствуют.

**Причинность.** Каждый обучающий якорь $A$ использует только события с
$d \le A$, а целевое окно $[A+1, A+30]$ отделено от обучения отступом,
равным горизонту: срез допустим при $T + 30 \le A$. Признаки на боевом
якоре 408 считаются той же функцией, что и на обучающих.

**Публичная часть.** Коэффициенты $\alpha$ при направлениях
восстанавливались по параболе $\mathrm{MSE}(\alpha)$ из замеров публичного
лидерборда — приём описан открыто в [EXPERIMENTS.md](EXPERIMENTS.md) вместе
со всеми замерами. Второй финальный слот — минимаксный хедж против того,
что часть этого прироста окажется специфичной для публичной выборки.
...
| **[PIPELINE.md](PIPELINE.md)** | порядок воспроизведения от `train.parquet` до финальных файлов | **повторить** результат |
...
| `submission_v29_blend18.csv` | 1.6477317 | $+0.22$ старого v18 — структурное разнообразие |
| `submission_v30_extrap22.csv` | 1.6475805 | экстраполяция прочь от v22 при половине $a^{*}$ |
| `submission_v31_vertex.csv` | 1.6475208 | шаг в вершину той же линии |
| `submission_v32_span20.csv` | 1.6474653 | направление к v20 при $a^{*}=-0.27$ |
| `submission_v33_gruprod.csv` | 1.6474152 | новое нейронаправление при $a=0.15$ — перелёт |
| `submission_v34_gru094.csv` | **1.6473878** | оно же в вершине $a^{*}=0.094$ — **лучший результат** |
| `submission_v35_gruens.csv` | 1.6473997 | усреднение трёх сидов — офлайн лучше, публично хуже |
| `submission_v36_btyd.csv` | 1.6472886 | направление BG/NBD-признаков |
| `submission_v37_hull2d.csv` | **1.6472485** | двумерная оболочка — **финальный слот 1** |
| `submission_final_hedge33.csv` | — | $\tfrac23$v23 $+\tfrac13$v37 — **финальный слот 2**, минимаксный хедж |
...
### Как собрать финал из того, что в репозитории
