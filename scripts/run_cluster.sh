...
for FOLD in 0 1; do
    ...
    python scripts/pretrain_gapgru.py --limit-anchor "$LIMIT" \
        ...
        --out "$OUT/pretrain_fold${FOLD}.pt" \
        2>&1 | tee "$OUT/logs/pretrain_f${FOLD}.log"
done

echo "### 3/3 этап B: остаток к ансамблю, два фолда"
python scripts/train_gapgru.py --max-len "$MAXLEN" --epochs "$EPOCHS_B" \
    --batch-size "$BS" --init-from "$OUT/pretrain" --freeze-epochs 2 \
    --ckpt "$OUT/gapgru_ckpt" --out "$OUT/gapgru.json" \
    2>&1 | tee "$OUT/logs/train.log"

echo "### результат"
python -c "
import json; r = json.load(open('$OUT/gapgru.json'))
